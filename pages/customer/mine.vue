		...
		<!-- 全部客户弹框 -->
		 <view class="allcusList" @click="closeAllcusListFn" v-if="isAllList">
			 <view class="allcusList_box">
				 ...
				 @click.stop="allcusListFn(index)"
				 :class="['allcusList_main',allcusListIndex==index?'activeallcusList_main':'']">
			...
			<view class="topmuieFixed_main">
				<view class="topmuieFixed_box">
					<view class="topmuieFixed_left" >
						<text @click.stop="topmuieLeft(index)" :class="topmuieLeftIndex===index?'topmuieFixed_lefttext':''"  v-for="(item,index) in topmuieFixed_left" :key="index">{{item}}</text>
					</view>
					<view class="topmuieFixed_right">
						<template v-if="topmuieFixed_righttext!=''">
						    <text @click.stop="topmuieRight(index)" :class="topmuieRightIndex===index?'topmuieFixed_righttext':''" v-for="(item,index) in topmuieFixed_righttext" :key="index">{{item.title}}</text>
						</template>
						<template v-if="topmuieLeftIndex===3||topmuieLeftIndex===4||topmuieLeftIndex===5||topmuieLeftIndex===6">
							<text @click.stop="topmuieRight(index)" :class="topmuieRightIndex===index?'topmuieFixed_righttext':''" v-for="(item,index) in topmuieFixed_rightDate" :key="index">{{item}}</text>
						</template>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			getCustomerList(params) {
				let _this = this;
				let url = 'http://crm.reactphp.club/api/customer/mine';
