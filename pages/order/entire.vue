<template>
	<view class="pages">
		...
		<view class="msg" v-for="(order,index) in orders" v-bind:key="order.id">
			<text class="msgtopright">{{index+1}}</text>
			<view @click="gotoOrderDetail(order.id)">
				<view class="msg_header">
					<view class="header_left">
						<text class="names">姓名： </text><text class="namemain">{{order.realname}}</text>
					</view>
					<view class="header_right">
						<text class="names">电话： </text><text class="namemain">{{order.mobile}}</text>
					</view>
				</view>
				<view class="msg_text">
					<text class="names">订单状态：</text> <text class="namemain">{{order.order_status}}</text>
				</view>
				<view class="msg_text">
					<text class="names">跟单客服：</text> <text class="namemain">{{order.sales_id}}</text>
				</view>
				<view class="msg_text">
					<text class="names">区域经理：</text> <text class="namemain">{{order.manager_id}}</text>
				</view>
				<view class="msg_text">
					<text class="names">签约日期：</text> <text class="namemain">{{order.sign_date}}</text>
				</view>
				<view class="msg_text">
					<text class="names">婚礼日期：</text> <text class="namemain">{{order.wedding_date}}</text>
				</view>
				<view class="msg_text">
					<text class="names">价格：</text> <text class="namemain">{{order.entire_price}}</text>
				</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			getOrderList() {
				let _this = this;
				let url = _this.$apis.order.entire;
				let params = {};
